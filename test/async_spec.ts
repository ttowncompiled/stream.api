...
import { Generator, OnComplete, OnNext } from '../src/types';
...
describe('Observable', () => {
  ...
  it('defer should produce a deferred observable', done => {
    ...
    let pending: number[] = [];
    let cb: Generator<number> = observer => sequence.forEach(item => observer.next(item));
    let observable: Observable<number> = Observable.defer<number>(cb);
    let subscriber: OnNext<number> = item => {
      if (pending.length === 0) observable.subscribeOnNext(subscriber);
      if (sequence.indexOf(item) > -1) {
        sequence.splice(sequence.indexOf(item), 1);
        pending.push(item);
      } else {
        expect(pending.indexOf(item)).to.be.above(-1);
...
describe('Cold Observable', () => {
  ...
  it('should emit all notifications to its subscription', done => {
    let sequence: number[] = [ 1, 2, 3 ];
    new ColdObservable<number>(observer => sequence.forEach(item => observer.next(item)))
      .subscribeOnNext(item => {
        expect(sequence.indexOf(item)).to.be.above(-1);
        sequence.splice(sequence.indexOf(item), 1);
  ...
  it('should emit all errors to its subscription', done => {
    let sequence: number[] = [ 1, 2, 3 ];
    new ColdObservable<number>(observer => sequence.forEach(item => observer.error(item)))
      .subscribeOnError(item => {
        expect(sequence.indexOf(item)).to.be.above(-1);
        sequence.splice(sequence.indexOf(item), 1);
        if (sequence.length === 0) done();
      });
...
describe('Deferred Observable', () => {
  ...
  it('should emit all errors to each of its subscriptions', done => {
    let sequence: number[] = [ 1, 2, 3 ];
    let pending: number[] = [];
    let cb: Generator<number> = observer => sequence.forEach(item => observer.error(item));
    let subscriber: OnNext<number> = item => {
      if (sequence.indexOf(item) > -1) {
        sequence.splice(sequence.indexOf(item), 1);
        pending.push(item);
