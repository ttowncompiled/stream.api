...
describe('Subject', () => {
  ...
  it('should subscribe to its following subscriptions if it has a subscriber', done => {
  ...
  it('if disposed, should be disposed after its subscriptions complete', done => {
    let subject: Subject<void> = new Subject<void>();
    subject.subscribeTo(Observable.create<void>(observer => observer.complete()));
  ...
  it('should listen for complete notifications and unsubscribe from its subscriptions', done => {
    let subject: Subject<void> = new Subject<void>();
    let observable: Observable<void> = Observable.create<void>(observer => observer.complete());
    subject.subscribeTo(observable);
    subject.subscribeOnComplete(() => {
  ...
  it('should listen for error notifications', done => {
    let subject: Subject<void> = new Subject<void>();
    subject.subscribeTo(Observable.create<void>(observer => observer.error(null)));
  ...
  it('should throw an error if subscribed to after being disposed', done => {
    let subject: Subject<void> = new Subject<void>();
    subject.dispose();
    try { subject.subscribeOnComplete(() => {}); }
    catch(err) { done(); }
  });
  
  it('should throw an error if a subscription is added after being disposed', done => {
    let subject: Subject<void> = new Subject<void>();
    subject.dispose();
    try { subject.subscribeTo(null); }
    catch(err) { done(); }
  });

  it('should throw an error if it calls complete after being disposed', done => {
    let subject: Subject<void> = new Subject<void>();
    subject.dispose();
